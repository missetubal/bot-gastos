...
from src.core.ai import (
    ...
    extract_correction_from_llama,
)  # <--- Certifique-se que extract_correction_from_llama está importado
...
async def _send_confirmation_message(
    ...
    categoria_nome_real = transaction_info.get(
        "categoria_nome_db"
    ) or transaction_info.get("categoria")
    if transaction_info.get("categoria_id") and not transaction_info.get(
        "categoria_nome_db"
    ):
        cat_info = next(
            (
                c
                for c in db.get_categorias(supabase_client)
                if c["id"] == transaction_info["categoria_id"]
            ),
            None,
        )
        if cat_info:
    ...
    forma_pagamento_nome_real = transaction_info.get(
        "forma_pagamento_nome_real"
    ) or transaction_info.get("forma_pagamento_text")
    if transaction_info.get("forma_pagamento_id") and not transaction_info.get(
        "forma_pagamento_nome_real"
    ):
        fp_info = next(
            (
    ...
    descricao_gasto_fmt = (
        f"({transaction_info.get('descricao_gasto', 'sem detalhes')})"
        if transaction_info.get("descricao_gasto")
        else ""
    )
...
async def _register_gasto(
    ...
    if db.add_gasto(
        supabase_client, valor, categoria_id, data, forma_pagamento_id, descricao_gasto
    ):
        await update.message.reply_text(
        ...
        if (
            ...
            if original_category_text.lower() not in [
                ...
                db.update_categoria_aliases(
                    supabase_client, categoria_id, list(current_aliases)
                )
                await update.message.reply_text(
...
async def handle_initial_message(
    ...
    parsed_info: Union[Dict[str, Any], None] = extract_transaction_info(
        user_message, supabase_client
    )
    ...
    if intencao == "adicionar_categoria":
        ...
        if not categoria_nome_input:
            await update.message.reply_text(
                "🤔 Não consegui identificar o nome da categoria que você quer adicionar."
            )
            return ConversationHandler.END

        if db.add_categoria(
            supabase_client, categoria_nome_input, limite_mensal=limite_mensal
        ):
            nome_exibicao = to_camel_case(categoria_nome_input)
            limite_msg = (
                f" com limite de R${limite_mensal:.2f}"
                if limite_mensal is not None and limite_mensal > 0
                else ""
            )
            await update.message.reply_text(
            ...
            nome_exibicao = to_camel_case(categoria_nome_input)
            await update.message.reply_text(
                f"⚠️ Erro ao adicionar categoria '{nome_exibicao}'. Ela já existe ou ocorreu um problema. Tente outro nome! 🧐"
            )
    ...
    # --- Lógica para Registrar Gasto ou Ganho (iniciar fluxo de confirmação) ---
        ...
        if intencao == "gasto":
            ...
            categoria_id = db.get_categoria_id_by_text(
                supabase_client, categoria_texto_llama
            )
            if categoria_id:
                context.user_data["pending_transaction"]["categoria_id"] = categoria_id
                context.user_data["pending_transaction"]["categoria_nome_db"] = next(
                    (
                        cat["nome"]
                        for cat in db.get_categorias(supabase_client)
                        if cat["id"] == categoria_id
                    ),
                    categoria_texto_llama,
                )
            else:
                similar_categories = db.find_similar_categories(
                    supabase_client, categoria_texto_llama
                )
                context.user_data["pending_transaction"]["suggestions"] = (
                    similar_categories
                )

                keyboard_options = [[cat["nome"]] for cat in similar_categories]
                keyboard_options.append(
                    ["Criar nova categoria ➕", "Não se aplica / Outra 🤷‍♀️"]
                )
                reply_markup = ReplyKeyboardMarkup(
                    keyboard_options, one_time_keyboard=True, resize_keyboard=True
                )
                await update.message.reply_text(
            ...
            if not forma_pagamento_id:
                ...
                await update.message.reply_text(
                    "💳 Qual foi a forma de pagamento?", reply_markup=reply_markup
                )
                return ASKING_PAYMENT_METHOD

            context.user_data["pending_transaction"]["forma_pagamento_id"] = (
                forma_pagamento_id
            )
        ...
        await _send_confirmation_message(
        ...
        "mostrar_balanco",
        "mostrar_grafico_gastos_categoria",
        "mostrar_grafico_gastos_por_pagamento",
        "mostrar_grafico_mensal_combinado",
    ]:
        ...
        if intencao == "mostrar_grafico_gastos_categoria":
            ...
            if forma_pagamento_text:
                ...
                if not forma_pagamento_id:
                    await update.message.reply_text(
                        f"⚠️ Forma de pagamento '{forma_pagamento_text}' não reconhecida. Gerando gráfico sem este filtro. 📊"
                    )
            ...
            if categoria_texto_llama:
                ...
                if not categoria_id:
                    await update.message.reply_text(
                        f"⚠️ Categoria '{categoria_texto_llama}' não reconhecida. Gerando gráfico sem este filtro. 📊"
                    )
            ...
            chart_buffer = charts.generate_balance_chart(
                supabase_client, data_inicio=data_inicio, data_fim=data_fim
            )
            ...
            await update.message.reply_text(
                "🤔 Não consegui identificar o tipo de gráfico. Use `/help` para ver as opções. 💡"
            )
            return ConversationHandler.END

        if chart_buffer:
            await update.message.reply_photo(
                photo=chart_buffer, caption=f"📊 Aqui está seu gráfico de {title}:"
            )
        else:
            await update.message.reply_text(
        ...
        if categoria_texto_llama:
            ...
            if not categoria_id:
                await update.message.reply_text(
                    f"⚠️ Categoria '{categoria_texto_llama}' não reconhecida. Listando todos os gastos no período."
                )
        ...
        if data_inicio and data_fim:
            ...
            if (
                start_date_obj.day == 1
                and end_date_obj.day
                == (end_date_obj + datetime.timedelta(days=1)).day - 1
                and start_date_obj.month == end_date_obj.month
                and start_date_obj.year == end_date_obj.year
            ):
                period_title = (
                    f" do mês de {start_date_obj.strftime('%B/%Y').capitalize()}"
                )
            else:
        ...
        if gastos_filtrados:
            ...
            for gasto in gastos_filtrados_sorted:
                ...
                forma_pagamento_nome = gasto.get(
                    "forma_pagamento_nome", "Não Informado"
                )
            ...
            await update.message.reply_text(
                f"Nenhum gasto encontrado{period_title} com os critérios fornecidos. 🤷‍♀️"
            )
        return ConversationHandler.END
    else:
        await update.message.reply_text(
...
async def handle_category_clarification(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> int:
    ...
    if chosen_category_id:
        ...
        context.user_data["pending_transaction"]["categoria_nome_db"] = (
        ...
        await update.message.reply_text(
            "📝 Ok, qual o nome da nova categoria para este gasto?",
            reply_markup=ReplyKeyboardRemove(),
        )
        ...
        await update.message.reply_text(
            "✍️ Entendi. Por favor, digite o nome da categoria correta para este gasto.",
            reply_markup=ReplyKeyboardRemove(),
        )
        return ASKING_NEW_CATEGORY_NAME
...
async def handle_new_category_name(
    ...
    if db.add_categoria(supabase_client, new_category_name_input, limite_mensal=None):
        ...
        categoria_id = db.get_categoria_id_by_text(
            supabase_client, new_category_name_camel_case
        )

        if categoria_id:
            context.user_data["pending_transaction"]["categoria_id"] = categoria_id
            context.user_data["pending_transaction"]["categoria_nome_db"] = (
                new_category_name_camel_case
            )
            ...
            await update.message.reply_text(
                f"⚠️ Não foi possível criar a categoria '{nome_existente_camel_case}'. Ela já existe ou houve um erro. Por favor, tente novamente ou escolha uma categoria existente. 🧐",
                reply_markup=ReplyKeyboardRemove(),
            )
            return ConversationHandler.END
...
async def handle_payment_method(
    ...
    if not forma_pagamento_id:
        if user_response_payment.lower() not in [
            ...
            try:
                response_add_fp = (
                    supabase_client.table("formas_pagamento")
                    .insert({"nome": final_payment_method_name})
                    .execute()
                )
...
async def handle_correction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if campo.lower() == "valor":
        ...
        nova_categoria_id = db.get_categoria_id_by_text(
        ...
        if novo_valor.lower() in ["gasto", "ganho"]:
            ...
            if novo_valor.lower() == "gasto":
                pending_transaction.pop(
                    "descricao", None
                )  # Remove 'descricao' de ganho
                if "categoria_id" not in pending_transaction:
                    pending_transaction["categoria_id"] = None
                ...
                if "descricao" not in pending_transaction:
                    pending_transaction["descricao"] = (
        ...
        if user_response_sub == "sim ✅" or user_response_sub == "sim":
            ...
            if new_category_name_from_correction:
                if db.add_categoria(
                    ...
                    new_cat_id = db.get_categoria_id_by_text(
