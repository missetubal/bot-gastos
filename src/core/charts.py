...
def generate_balance_chart(
    ...
    monthly_summary = df_all.groupby(['mes_ano', 'tipo'])['value'].sum().unstack(fill_value=0)
...
def generate_category_spending_chart(
    ...
    try:
        gastos_data_raw = supabase_client.table('expenses').select('value,category_id,date,payment_methods(name),categories(name,monthly_limit),payment_method_id').execute().data
        categorias_data_full = supabase_client.table('categories').select('id,name,monthly_limit').execute().data
    except Exception as e:
    ...
    for gasto in gastos_data_raw:
        ...
        if 'categories' in gasto_copy and gasto_copy['categories'] and 'name' in gasto_copy['categories']:
            ...
            gasto_copy['limite_mensal_categoria'] = gasto_copy['categories']['monthly_limit']
        else:
    ...
    gastos_por_categoria = df_gastos.groupby('categoria_nome')['value'].sum().sort_values(ascending=False)

    if gastos_por_categoria.empty:
        return None

    limites_por_categoria = {cat['name']: cat['monthly_limit'] for cat in categorias_data_full}
    ...
    if forma_pagamento_id:
        fp_info = supabase_client.table('payment_methods').select('name').eq('id', forma_pagamento_id).limit(1).execute().data
        if fp_info: payment_filter_title = f" por {fp_info[0]['name']}"
...
def generate_payment_method_spending_chart(supabase_client: Client, 
    ...
    gastos_por_forma = df_gastos.groupby('forma_pagamento_nome')['value'].sum().sort_values(ascending=False)
    ...
    if category_id:
        cat_info = supabase_client.table('categories').select('name').eq('id', category_id).limit(1).execute().data
        if cat_info: category_filter_title = f" em {cat_info[0]['name']}"
    period_title = _get_period_title(data_inicio, data_fim)
