...
def generate_balance_chart(
    ...
    try:
        gastos_data_raw = (
            supabase_client.table("gastos").select("valor,data").execute().data
        )
        ganhos_data_raw = (
    ...
    monthly_summary = (
        df_all.groupby(["mes_ano", "tipo"])["valor"].sum().unstack(fill_value=0)
    )
...
def generate_category_spending_chart(
    ...
    try:
        gastos_data_raw = (
            ...
            .select(
                "valor,categoria_id,data,formas_pagamento(nome),categorias(nome,limite_mensal),forma_pagamento_id"
            )
            .execute()
            .data
        )
    ...
    for gasto in gastos_data_raw:
        ...
        if (
            ...
            gasto_copy["limite_mensal_categoria"] = gasto_copy["categorias"][
                "limite_mensal"
            ]
        else:
    ...
    gastos_por_categoria = (
        df_gastos.groupby("categoria_nome")["valor"].sum().sort_values(ascending=False)
    )
    ...
    limites_por_categoria = {
        cat["nome"]: cat["limite_mensal"] for cat in categorias_data_full
    }
    ...
    if forma_pagamento_id:
        ...
        if fp_info:
            payment_filter_title = f" por {fp_info[0]['nome']}"
...
def generate_payment_method_spending_chart(
    ...
    try:
        gastos_data_raw = (
            supabase_client.table("gastos")
            .select("valor,data,formas_pagamento(nome),categoria_id")
            .execute()
            .data
        )
    except Exception as e:
    ...
    gastos_por_forma = (
        df_gastos.groupby("forma_pagamento_nome")["valor"]
        .sum()
    ...
    if categoria_id:
        ...
        if cat_info:
            category_filter_title = f" em {cat_info[0]['nome']}"
    period_title = _get_period_title(data_inicio, data_fim)
...
def generate_monthly_category_payment_chart(
    ...
    try:
        gastos_data_raw = (
            supabase_client.table("gastos")
            .select("valor,data,formas_pagamento(nome),categorias(nome)")
            .execute()
            .data
        )
    except Exception as e:
