...
def get_forma_pagamento_id_by_name(
    supabase_client: Client, nome: str
) -> Union[str, None]:
...
def get_gastos(supabase_client: Client) -> list:
    ...
    try:
        response = (
            supabase_client.table("gastos")
            .select(
                "valor,categoria_id,data,descricao,formas_pagamento(nome),categorias(nome)"
            )
        ...
        for gasto in response.data:
            ...
            if (
                "categorias" in gasto_copy
                and gasto_copy["categorias"]
                and "nome" in gasto_copy["categorias"]
            ):
            ...
            if (
                "formas_pagamento" in gasto_copy
                and gasto_copy["formas_pagamento"]
                and "nome" in gasto_copy["formas_pagamento"]
            ):
                gasto_copy["forma_pagamento_nome"] = gasto_copy["formas_pagamento"][
                    "nome"
                ]
            else:
...
def get_gastos_by_category(supabase_client: Client, categoria_id: str) -> list:
    ...
    try:
        ...
        response = (
            supabase_client.table("gastos")
            .select("valor,data,descricao,formas_pagamento(nome)")
...
def add_ganho(supabase_client: Client, valor: float, descricao: str, data: str) -> bool:
    ...
    try:
        response = (
            ...
            .insert({"valor": valor, "descricao": descricao, "data": data})
            .execute()
        )
        return True
    except Exception as e:
...
def add_categoria(
    supabase_client: Client,
    nome: str,
    limite_mensal: Union[float, None] = None,
    ...
    try:
        existing_category = (
            supabase_client.table("categorias")
            .select("id")
            .eq("nome", nome_camel_case)
            .execute()
            .data
        )
...
def get_categorias(supabase_client: Client) -> list:
    ...
    try:
        response = (
            supabase_client.table("categorias")
            .select("id,nome,limite_mensal,aliases")
            .order("nome")
            .execute()
        )
        return response.data
...
def get_categoria_id_by_text(
    supabase_client: Client, text_from_llama: str
) -> Union[str, None]:
    """
...
def find_similar_categories(supabase_client: Client, text: str) -> List[Dict[str, Any]]:
    ...
    llama_suggestion_name = suggest_category_from_llama(
        text, existing_category_names, supabase_client
    )
    ...
    for cat in categorias_do_banco:
        if (
            text_lower in cat["nome"].lower()
            or cat["nome"].lower() in text_lower
            or text_camel_case in cat["nome"]
            or cat["nome"] in text_camel_case
        ):
            similar_cats.append({"id": cat["id"], "nome": cat["nome"]})
...
def update_categoria_limite(
    supabase_client: Client, categoria_id: str, novo_limite: Union[float, None]
) -> bool:
...
def update_categoria_aliases(
    supabase_client: Client, categoria_id: str, new_aliases: List[str]
) -> bool:
