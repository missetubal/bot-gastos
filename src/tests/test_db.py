...
class TestDatabase(unittest.TestCase):
    ...
    def test_add_gasto_success(self):
        ...
        self.mock_table_methods.insert.return_value.execute.return_value = MagicMock(data=[{'id': str(uuid.uuid4())}]) 
        result = db.add_gasto(self.mock_supabase_client, 100.0, str(uuid.uuid4()), "2025-07-01", str(uuid.uuid4()), "Compras no mercado")
    ...
    def test_add_gasto_failure(self):
        ...
        result = db.add_gasto(self.mock_supabase_client, 100.0, str(uuid.uuid4()), "2025-07-01")
    ...
    def test_get_gastos_with_data(self):
        mock_data = [
            {'valor': 50.0, 'categoria_id': 'cat1', 'data': '2025-07-01', 'descricao': 'Cafe',
             'formas_pagamento': {'nome': 'Pix'}, 'categorias': {'nome': 'Alimentacao'}},
        ]
    ...
    def test_add_categoria_success(self, mock_to_camel_case):
        ...
        result = db.add_categoria(self.mock_supabase_client, "nova categoria", 500.0, ["nova", "cat"])
        self.assertTrue(result)
    ...
    def test_get_categorias_with_data(self):
        mock_data = [
            {'id': 'cat1', 'nome': 'Alimentacao', 'limite_mensal': 800.0, 'aliases': ['food', 'grocery']},
            {'id': 'cat2', 'nome': 'Transporte', 'limite_mensal': None, 'aliases': None}
        ]
    ...
    def test_get_categoria_id_by_text_exact_name(self, mock_to_camel_case, mock_get_categorias):
        mock_get_categorias.return_value = [{'id': 'cat1', 'nome': 'Alimentacao', 'aliases': []}]
        cat_id = db.get_categoria_id_by_text(self.mock_supabase_client, "Alimentacao")
        self.assertEqual(cat_id, 'cat1')

    @patch('src.core.db.get_categorias')
    @patch('src.utils.text_utils.to_camel_case', side_effect=lambda x: ''.join(word.capitalize() for word in x.split()))
    def test_get_categoria_id_by_text_alias(self, mock_to_camel_case, mock_get_categorias):
        mock_get_categorias.return_value = [{'id': 'cat1', 'nome': 'Alimentacao', 'aliases': ['mercado', 'comida']}]
        cat_id = db.get_categoria_id_by_text(self.mock_supabase_client, "mercado")
        self.assertEqual(cat_id, 'cat1')

    @patch('src.core.db.get_categorias')
    @patch('src.utils.text_utils.to_camel_case', side_effect=lambda x: ''.join(word.capitalize() for word in x.split()))
    def test_get_categoria_id_by_text_not_found(self, mock_to_camel_case, mock_get_categorias):
        mock_get_categorias.return_value = [{'id': 'cat1', 'nome': 'Alimentacao', 'aliases': []}]
    ...
    def test_get_forma_pagamento_id_by_name_success(self):
        self.mock_table_methods.select.return_value.execute.return_value.data = [{'id': 'fp1', 'nome': 'Pix'}]
    ...
    def test_update_categoria_limite_success(self):
        self.mock_table_methods.update.return_value.eq.return_value.execute.return_value = MagicMock(data=[{'id': 'cat1', 'limite_mensal': 100.0}])
    ...
    def test_update_categoria_aliases_success(self):
        ...
        result = db.update_categoria_aliases(self.mock_supabase_client, 'cat1', ['alias1'])
        self.assertTrue(result)