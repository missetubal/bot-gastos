...
class TestArtificialIntelligence(unittest.TestCase):
    def setUp(self):
        ...
        self.yesterday_str = (
            datetime.date(2025, 7, 7) - datetime.timedelta(days=1)
        ).strftime("%Y-%m-%d")
        self.two_days_ago_str = (
            datetime.date(2025, 7, 7) - datetime.timedelta(days=2)
        ).strftime("%Y-%m-%d")
        self.last_week_day_str = (
            datetime.date(2025, 7, 7) - datetime.timedelta(days=7)
        ).strftime("%Y-%m-%d")
        self.current_month_start_str = datetime.date(2025, 7, 1).strftime("%Y-%m-%d")
        self.current_month_end_str = datetime.date(2025, 7, 31).strftime(
            "%Y-%m-%d"
        )  # Assumindo 31 dias para o mês
    ...
    def test_ask_llama_success(self, mock_post):
        ...
        mock_post.assert_called_once_with(
            ...
            data=json.dumps(
                {"model": ai.OLLAMA_MODEL, "prompt": "Hello Llama", "stream": False}
            ),
        )
    ...
    def test_ask_llama_failure(self, mock_post):
        ...
        self.assertEqual(
            response,
            "Desculpe, não consegui processar sua requisição agora. O Llama está offline ou o modelo não está disponível?",
        )
